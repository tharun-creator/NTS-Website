...
export default function CartDrawer({ isOpen, onClose, cartItems, removeFromCart }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const subject = encodeURIComponent(`B2B Inquiry from ${formData.company}`)
    const body = encodeURIComponent(
      `Name: ${formData.name}\nEmail: ${formData.email}\nCompany: ${formData.company}\nItems Requested: ${cartItems.map((item) => item.name).join(', ')}\n\nNotes:\n${formData.notes}`
    )
      ...
      <div className={`absolute right-0 top-0 bottom-0 w-full max-w-md bg-cream text-maroon flex flex-col shadow-2xl transition-transform duration-500 ease-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-6 border-b border-maroon/10 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-maroon/80 uppercase tracking-widest font-sans font-bold">Manufacturing and Sample Requests</p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="space-y-3">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4 items-center bg-white p-3 rounded-2xl shadow-sm border border-maroon/5">
                    {item.image ? (
                      <img src={item.image} alt={item.name} className="h-12 w-10 object-contain shrink-0" />
                    ) : (
                      <span className="h-10 w-10 rounded-full bg-cream text-maroon text-[10px] font-bold flex items-center justify-center shrink-0">
                        NTS
                      </span>
                    )}
